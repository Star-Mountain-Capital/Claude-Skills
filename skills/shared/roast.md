...
# Roast — Adversarial Idea Pressure Testing

Claude's default is to agree with you. `/roast` is the opposite. It convenes a council of five independent personas who tear an idea apart and build it up from every angle, then a Judge synthesizes everything into one honest verdict. Use it before you sink time and money into building the wrong thing.
...
If the user provides the idea in their message, start there. Then ask a tight set of clarifying questions so the council has real context to work with. Ask only what hasn't already been provided. Keep it to 3–4 questions max, in one batch:
...
## Step 2: Convene the council (5 personas)

In Claude Code, spin up all five personas as parallel agents in a single message (one agent call each). In claude.ai, run them sequentially in the same conversation, clearly labeling each persona and keeping them fully in character.

Paste the same brief into each persona, then apply their mandate:
...
Once all five have weighed in, act as the Judge. Read every council member's findings, weigh them, and synthesize one decisive verdict. Do not just average the scores. Name the real tension between the personas and resolve it.

Fold in the **economics lens**: rough pricing, realistic time-to-first-dollar, and whether the user can actually ship this given the edge they described.
...
This skill is especially useful at SMC for:
- Pressure-testing a new automation or product idea before building it in Meridian or n8n
- Stress-testing a new fund strategy, product structure, or investment thesis before IC
- Validating a new operational workflow before committing firm resources
- Getting an adversarial read on a technology vendor or platform before signing a contract
- Testing a new LP communication strategy or fundraising approach before rollout

## Example Requests

```
/roast Build a borrowing base automation tool in n8n that pulls portfolio company financials from Notion and calculates availability daily, replacing the manual Excel process.
```

```
Roast this idea: we create a standardized LP onboarding portal on Meridian that replaces the current email-and-PDF process. LPs log in, submit KYC docs, sign subscription docs via DocuSign, and track their capital call status.
```

```
Stress-test this: we move all fund reporting from the external administrator's PDF statements to a live Notion dashboard we maintain ourselves, synced via n8n.
```
