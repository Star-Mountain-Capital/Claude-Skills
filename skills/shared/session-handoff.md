...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `%USERPROFILE%\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`%USERPROFILE%\.claude\projects\<project>\memory\`).
...
- Writing the summary to a file anywhere. This is chat-only by design.
